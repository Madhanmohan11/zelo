...
export const EditSloganModal = ({ isOpen, onClose, currentSlogan = '', onSaveSuccess = () => {} }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast('Personal slogan updated!', 'success')
      onSaveSuccess(clean)
      onClose()
    } catch (err) {
      console.error('Failed to save slogan:', err)
      showToast('Failed to save slogan', 'error')
    } finally {
      setIsSaving(false)
    }
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <p className="text-xs font-semibold text-slate-500 mb-3">
            Write a short phrase that keeps you motivated every day.
          </p>
          ...
          <div className="relative">
            <textarea
              rows={3}
              maxLength={60}
              placeholder="e.g. A better you, every day."
              value={slogan}
